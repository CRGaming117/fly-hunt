...
for i in range(20):
    flies.append(
        Fly(
            random.randint(20, surface.get_width() - 20),
            random.randint(20, surface.get_height() - 20),
            10,
            BLACK,
        )
    )
...
winMsg.setLoc(
    (surface.get_width() // 2) - (winMsg.get_rect().width // 2),
    (surface.get_height() // 2) - (winMsg.get_rect().height // 2),
)
...
while running:
        ...
        if (not loseMsg.isVisible()) or (not winMsg.isVisible()):
            ...
            if fly.getColor() == RED and fly.isVisible():
                ...
                if fly.getXSpeed() == 0 or fly.getYSpeed() == 0:
                    d = 6
                    pygame.draw.circle(
                        surface, BLACK, (fly.getX() - d, fly.getY() - d), s
                    )
                    pygame.draw.circle(
                        surface, BLACK, (fly.getX() + d, fly.getY() - d), s
                    )
    ...
    if not frog1.isVisible():
        ...
        pygame.draw.line(
            surface,
            PINK,
            (frog1.getX(), frog1.getY() - frog1.getSize()),
            (frog1.getX(), frog1.getY() + frog1.getSize()),
            2,
        )
        pygame.draw.line(
            surface,
            PINK,
            (frog1.getX() - frog1.getSize(), frog1.getY()),
            (frog1.getX() + frog1.getSize(), frog1.getY()),
            2,
        )
    else:
    ...
    pygame.draw.circle(
        ...
        GREEN,
        (
            (surface.get_width() // 2) - (povSize // 2),
            surface.get_height() - int(povSize * 0.75),
        ),
        int((povSize // 2.5)),
    )
    pygame.draw.circle(
    ...
    pygame.draw.circle(
        surface, GREEN, (surface.get_width() // 2, surface.get_height()), povSize
    )
